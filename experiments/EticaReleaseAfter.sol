...
/**
     ...
     */
function readjustThreshold10() public {
    uint initial = periodsCounter - PERIODS_PER_THRESHOLD;
    uint initialSum = _totalfor; 
    uint loopcondition = periodsCounter-1;
    _totalfor = readjustThreshold1_for(initial, initialSum, loopcondition);
  }

  /**
     * @notice postcondition temp_total >= __verifier_old_uint(temp_total)
     * @notice postcondition loopcondition != periodsCounter-1 || initial != 0 || val == initialSum + __verifier_sum_uint(periods.forprops)
     */
  function readjustThreshold1_for(uint initial, uint initialSum, uint loopcondition) internal returns (uint) {
    uint temp_total = initialSum;
        /**
         * @notice invariant temp_total >= __verifier_old_uint(temp_total)
         * @notice invariant temp_total == __verifier_old_uint(temp_total) + sum(periods[0].forprops ... periods[i].forprops)
         */
    for(uint i=initial; i < loopcondition;i++){
      temp_total = temp_total + periods[i].forprops;
    }
    return temp_total;
  } 
  ...
  /**
     ...
     * @notice postcondition _tot_totalagainstalfor == __verifier_old_uint(_totalagainst) + __verifier_sum_uint(periods.forprops)
     */
  function readjustThreshold20() public {
    uint initial = periodsCounter - PERIODS_PER_THRESHOLD;
    uint initialSum = _totalagainst; 
    uint loopcondition = periodsCounter-1;
    _totalagainst = readjustThreshold2_for(initial, initialSum, loopcondition);
  }
  ...
  /**
     ...
     * @notice postcondition loopcondition != periodsCounter-1 || initial != 0 || val == initialSum + __verifier_sum_uint(periods.forprops)
     */
  function readjustThreshold2_for(uint initial, uint initialSum, uint loopcondition) internal returns (uint) {
    uint temp_total = initialSum;
    /**
         * @notice invariant temp_total >= __verifier_old_uint(temp_total)
         * @notice invariant temp_total == __verifier_old_uint(temp_total) + sum(periods[0].forprops ... periods[i].forprops)
         */
    for(uint i=initial; i < loopcondition;i++){
      temp_total = temp_total + periods[i].againstprops;
    }
    return temp_total;
  } 
}
