contract SRules {
    ...
    uint start; 
    uint end;
    uint walletDetailsCount;
    uint downlineCounts = 0;
    function initpoolBalance() public {
    uint rvariable = 0; 
    uint initial = 0;
    uint loopcondition = walletDetailsCount;
    initpoolBalance_for(rvariable, initial, loopcondition);
  }
  function initpoolBalancefor(bool rvariable, uint initial, uint loopcondition) internal {
  for (uint i = initial; i < loopcondition; i++) {
    poolBalance[i] = rvariable;
   }
  }

    function distSales() public {
    uint initial = 1;
    uint initialSum = transferAmount; 
    uint loopcondition = walletDetailsCount;
    transferAmount = distSales_for(initial, initialSum, loopcondition);
  }
  function distSales_for(uint initial, uint initialSum, uint loopcondition) internal returns (uint) {
    uint temp_total = initialSum;
    for(uint i=initial; i < loopcondition;i++){
      temp_total = temp_total * walletDetails[i].percentage;
    }
    return temp_total;
  }  
}